...
import { AppProvider, Page, Card, Button, Stack, Modal, ResourceList } from '@shopify/polaris';
...
class ProductEdit extends Component {
  ...
  addBlock(image_url) {
    ...
    this.setState({
      blocks: [
        ...this.state.blocks,
        {
          block_id: Math.round(Math.random() * 1000),
          block_text: "new block",
          block_type: this.state.currentBlockType,
          image_url: image_url
        }
      ]
    })
  ...
  saveLookbook() {
    ...
    this.setState({ stored_product }, () => {
      const tokens = document.querySelector("meta[name='csrf-token']").getAttribute("content");
        ...
        >
          ...
          >
            <Stack distribution="fill">
              <Button size="medium" onClick={() => this.openModal('Block3')}>
                {static_text.block3_title}
              </Button>
              <Button size="medium" onClick={() => this.openModal('Block1')}>
                {static_text.block1_title}
              </Button>
              <Button size="medium" onClick={() => this.openModal('Block2')}>
                {static_text.block2_title}
              </Button>
              <Button destructive size="medium" onClick={() => this.deleteLastBlock()}>
